...
from vmBound import main_vm_bound

# ------ preparation ------ #

timeStamp_now = str(pd.Timestamp.now())
dir_name = "./vmBound Test Result " + timeStamp_now
# create a directory
...
ads_num = 8
numFramesIterates = [4, 6, 8]
rangePagesIterates = [4, 5, 6, 7, 8]
occupancyIterates = [0,1,2,3,4]
...
for pageOffset in [pageOffset1]:
    ...
    for occupancy in occupancyIterates:
        for num_frames in numFramesIterates:
            for range_pages in rangePagesIterates:
                for index_bits, offset_bits in pageOffset:
                    ...
                    for i in range(num_reps):
                        ...
                        currAlgo = main_vm_bound(index_bits, offset_bits, ads_num, num_frames, range_pages, occupancy)
                    ...
                    # create a directory
                    file_gen_name = dir_name + subfolder + "/" + currAlgo.name + "_result" + timeStamp_now + "_index" + str(index_bits) + "_offset" + str(offset_bits) + "_adsnum" + str(ads_num) + "_reps" + str(num_reps) + "_numFrames" + str(num_frames) + "_rangePages" + str(range_pages) + "_occupancy" + str(occupancy)
    ...
    file_acrossAlgo_name = dir_name + "/vmbound analysis ads_len=" + str(ads_len) + ".csv"
    file_plot_name = dir_name + "/vmbound analysis plot ads_len=" + str(ads_len) + ".pdf"
